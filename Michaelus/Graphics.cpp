...
void Graphics::UpdateUniformBuffer(uint32_t currentImage)
{
    ...
    ubo.model = glm::rotate(glm::mat4(1.f), time * MMath::Rad(30.f), glm::vec3(0.f, 0.f, 1.f));
...
void Graphics::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling,
	VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
    ...
    if (VK_FAILED(vkCreateImage(vkDevice, &imageInfo, nullptr, &vkTextureImage)))
    ...
    if (VK_FAILED(vkAllocateMemory(vkDevice, &allocInfo, nullptr, &vkTextureImageMemory)))
